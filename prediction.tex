...
To pick the best ACS adaptively, we take a data-driven approach to exhaustively search for the best ACS. \xil{i am confused, if computation cost is high, how to justify exhaustive search?} We separate part of historical sessions as test set. Then, the best ACS is the one that gives the minimal overall prediction error over the test set using WIMSE with this ACS.
...
\xil{flow from beginning of sec 5 to this point is VERY CONFUSING}
...
\begin{packeditemize}
	\item CDN+Bitrate AC\jc{need a better name for this}: Predict using average of each CDN and bitrate combination: picking the decision that is globally best (i.e., no spatial partition) \xil{mention this is coarse}
