...
\myparatight{Large configuration space} as protocols and applications become more sophisticated the number of choices increases dramatically. For instance, with a video application one can select the initial bit-rate, the CDN, and at a finer granularity the proxy or the web server from which to stream the content. As the number of decisions and the number of attributes (characteristics) impacting the performance increases it might be infeasible for a reactive protocol to search even a small fraction of the configuration space.
...
Third, given the large number of attributes that impact a session performance (e.g., protocol, device, OS, firmware, ISP, geographic location, to name a few) even if we have access to a very large number of sessions, it is hard to find enough sessions that match all the attributes of the sessions whose performance we wish to predict. To address this challenge we argue that we need to \emph{aggregate} sessions with similar but not exact attributes, and that the attributes along we perform the aggregation are determinant in improving the prediction accuracy.
