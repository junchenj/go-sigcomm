...
While in previous section we showed that video metrics are predictable with the attributes we collected, there are still several challenges in designing such prediction algorithm in practice. 

We examine the challenges faced by a straightforward approach in which the prediction is the mean of quality of quality samples from the sessions that match all attributes to the session under prediction.
In particular, while the lower bound tends to decrease with more attributes, the data points (quality samples) contained in each fine-grained identical group\footnote{In the case of quality sample, an identical group consists of quality samples whose session matches all value in an AC.} becomes {\it sparse}, and such sparsity is susceptible to noise. Moreover, a practical algorithm inherently has a processing delay so that the session under prediction can only be predicted using information that was collected a while ago.
...
\myparatight{Optimal AC} We use a simple methodology, called {\it optimal AC}, to show the dynamics of the right degree aggregation. We assume that the information (quality samples) collected before this minute can only be used for predicting sessions in the next minute.
For a finest identical group at $t$-th minute, we first build a hierarchy that consists of all degrees of aggregation over the quality samples we collects before the $t$-th minute. Figure~\ref{fig:example-optimal-ac} shows an example of three attributes. 
...
{
        \includegraphics[width=0.22\textwidth]{figures/optimal-prevalence.pdf}
}
\subfigure[Optimal AC persistence.]
{
        \includegraphics[width=0.22\textwidth]{figures/optimal-persistence.pdf}
}
...
To conclude, we find that the optimal AC (the aggregation that gives minimal prediction error) is in many cases neither the finest nor the coarsest one, and, in addition, is highly dynamic:
